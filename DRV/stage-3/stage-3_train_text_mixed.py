...
parser.add_argument("--dataset", type=str, default="/home/felixchaotw/mllm-physical-design/armo/dataset/train_desc_mixed.csv")
...
for epoch in tqdm(range(num_epochs), desc="Training Epochs"):
    ...
    for batch in train_loader:
        ...
        print(f"Steps: {steps}, Batch Loss: {loss.item()}")
        ...
        if steps % 1000 == 0:
            model.eval()
            torch.save(model.state_dict(), f"/data1/felixchao/output/checkpoint-{epoch}-mixed.pth")
            print("Model saved")
        
    # print(f"Epoch {epoch + 1}/{num_epochs}, Loss: {epoch_loss / len(train_loader)}")  
     
    # model.eval()
    # torch.save(model.state_dict(), f"/data1/felixchao/output/checkpoint-{epoch}-log.pth")
    # print("Model saved")
