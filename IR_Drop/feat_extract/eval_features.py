...
def evaluate_design(df, save_path, baseline=False, design_name="zero-riscy-a"):
    ...
    if baseline:
        ...
        x = list(congestion_set.keys())[::50]
        x = [name.split("-")[0] for name in x]
        x_label = list(range(0,len(x)))
        y = [predicted[file_path] for file_path in congestion_set.keys()][::50]
        y_gpdl = [predicted_gpdl[file_path] for file_path in congestion_set.keys()][::50]
        ...
        plt.legend(["GARF", "MAVI", "Ground Truth"])
...
def get_gpdl_prediction(design, feature_path, model, device):
     ...
     for filename in tqdm(os.listdir(feature_path)):
          ...
          with torch.no_grad():
               input_image = torch.tensor(batch_image).unsqueeze(0).unsqueeze(1).float().to(device)
               output_image = model(input_image)
               prediction = get_ir_drop(output_image.cpu().numpy().squeeze())
...
def main():
    ...
    parser.add_argument("--pretrained", type=str, default="/home/felixchaotw/CircuitNet/model/IR_drop.pth")
    parser.add_argument("--feature_path", type=str, default="/data2/NVIDIA/CircuitNet-N28/Dataset/IR_drop/feature/")
    parser.add_argument("--label_path", type=str, default="/data2/NVIDIA/CircuitNet-N28/Dataset/IR_drop/label/")
